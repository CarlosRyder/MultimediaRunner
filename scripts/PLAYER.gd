...
func _ready():
	cam_position = $ANCHOR/MESH/Camera3D.transform.origin
	look_at_direction = global_transform.basis.z
	cam_transform = $ANCHOR/MESH/Camera3D.transform
	...
	$ANCHOR/MESH/MODEL/SENSOR.connect("body_entered", Callable(self, "on_collision"))
	$ANCHOR/MESH/MODEL/MAGNET_SENSOR.connect("body_entered", Callable(self, "on_magnet_collision"))
	Globals.connect("on_die", Callable(self, "on_die"))
	Globals.connect("on_toggle_magnet", Callable(self, "on_toggle_magnet"))

func _process(delta):
	# camera shake
	if shake_intensity > 0.0:
		shake_intensity -= delta
		$ANCHOR/MESH/Camera3D.transform.origin = cam_position + 2.0 * shake_intensity * Vector3(cos(0.05 * OS.get_system_time_msecs()), sin(0.05 * OS.get_system_time_msecs()), 0.0)
	...
	if shake_intensity <= 0.0:
		$ANCHOR/MESH/Camera3D.transform.origin.x = 0.35 * cos(0.0075 * OS.get_system_time_msecs())
		$ANCHOR/MESH/Camera3D.transform.origin.y = cam_position.y + 0.35 * sin(0.0075 * OS.get_system_time_msecs())
...
func on_toggle_magnet(activate):
	...
	$ANCHOR/MESH/MODEL/bee/rig/Skeleton3D/MAGNET.visible = activate
